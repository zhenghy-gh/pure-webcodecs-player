...
2. **第 2 轮触发条件**：M3 区块（t18 管线/t19 demo/t20 汇总页）✅ → 输出 `docs/review/round-2-问题清单.md`。
   **✅ 已触发并输出（2026-09-09）**：t18 管线（§43 WebCodecs 15 步 + §44 MSE 19 步）、t19 demo（§48 16/16 真机）、t20 汇总页（§47）三项均达成 → `docs/review/round-2-问题清单.md` 已产出（条目级汇总：阻断 17 / 严重 23 / 建议 6，全部闭环；逐波明细仍见 `round-1-问题清单.md` §28–§51）。
